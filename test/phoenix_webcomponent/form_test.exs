defmodule Phoenix.WebComponent.FormTest do
      ...
      form =
        safe_to_string(
          form_for(conn, "/", [as: :search], fn f ->
            assert f.impl == Phoenix.WebComponent.FormData.Plug.Conn
        ...
        safe_to_string(
          form_for(conn(), "/", [errors: errors], fn f ->
            for {field, {message, _}} <- f.errors do
              Phoenix.WebComponent.Tag.content_tag(:span, humanize(field) <> " " <> message,
                class: "errors"
              )
            end
          end)
      ...
      form =
        safe_to_string(
          form_for(:search, "/", fn f ->
            assert f.impl == Phoenix.WebComponent.FormData.Atom
            assert f.name == "search"
  ...
  describe "inputs_for/3" do
    ...
    test "support atom or binary field" do
      form = Phoenix.WebComponent.FormData.to_form(:user, [])
  ...
  describe "hidden_inputs_for/1" do
    test "generates hidden fields from the given form" do
      form = %{Phoenix.WebComponent.FormData.to_form(conn(), []) | hidden: [id: 1]}

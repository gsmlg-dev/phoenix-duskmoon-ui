defmodule Phoenix.WebComponent do
  ...
  @type unsafe :: Phoenix.WebComponent.Safe.t()
  ...
  @spec sigil_E({:<<>>, nil | maybe_improper_list | map, [binary, ...]}, []) ::
          atom
          | binary
          | list
          | number
  ...
  def html_escape(other), do: {:safe, Phoenix.WebComponent.Engine.encode_to_iodata!(other)}
  ...
  implement the `Phoenix.WebComponent.Safe` protocol. In addition,
  ...
  at `priv/static/phoenix_html.js`. See the `Phoenix.WebComponent` module
    ...
    data =
      if method == :get do
        ...
        case Keyword.get(opts, :csrf_token, true) do
          true -> [csrf: Phoenix.WebComponent.Tag.csrf_token_value(to)] ++ data
          false -> data

defmodule Phoenix.WebComponent.Engine do
  @moduledoc """
  ...
  defp to_safe(literal, _line)
    ...
    literal
    |> Phoenix.WebComponent.Safe.to_iodata()
    |> IO.iodata_to_binary()
  end

  # We can do the work at runtime
  defp to_safe(literal, line) when is_list(literal) do
    quote line: line, do: Phoenix.WebComponent.Safe.List.to_iodata(unquote(literal))
  end
  ...
  defp to_safe(expr, line) do
    ...
    bin_return = quote line: line, do: Phoenix.WebComponent.Engine.html_escape(bin)
    other_return = quote line: line, do: Phoenix.WebComponent.Safe.to_iodata(other)
  ...
  defp handle_assign({:@, meta, [{name, _, atom}]}) when is_atom(name) and is_atom(atom) do
    quote line: meta[:line] || 0 do
      Phoenix.WebComponent.Engine.fetch_assign!(var!(assigns), unquote(name))
    end
  end

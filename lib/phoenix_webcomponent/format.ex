defmodule Phoenix.WebComponent.Format do
  @moduledoc """
  ...
  """
  @spec text_to_html(Phoenix.WebComponent.unsafe(), Keyword.t()) :: Phoenix.WebComponent.safe()
  def text_to_html(string, opts \\ []) do
    ...
    |> Phoenix.WebComponent.html_escape()
  end

  defp maybe_html_escape(string, true),
    do: string |> Phoenix.WebComponent.Engine.html_escape() |> IO.iodata_to_binary()
  ...
  defp wrap_paragraph(text, tag, attributes, insert_brs?) do
    [Phoenix.WebComponent.Tag.content_tag(tag, insert_brs(text, insert_brs?), attributes), ?\n]
  end
  ...
  defp insert_brs(text, false) do
    ...
    |> split_lines()
    |> Enum.intersperse(?\s)
    |> Phoenix.WebComponent.raw()
  end

  defp insert_brs(text, true) do
    text
    |> split_lines()
    |> Enum.map(&Phoenix.WebComponent.raw/1)
    |> Enum.intersperse([Phoenix.WebComponent.Tag.tag(:br), ?\n])
  end

defprotocol Phoenix.WebComponent.Safe do
  @moduledoc """
...
defimpl Phoenix.WebComponent.Safe, for: Atom do
  def to_iodata(nil), do: ""
  def to_iodata(atom), do: Phoenix.WebComponent.Engine.html_escape(Atom.to_string(atom))
end

defimpl Phoenix.WebComponent.Safe, for: BitString do
  defdelegate to_iodata(data), to: Phoenix.WebComponent.Engine, as: :html_escape
end

defimpl Phoenix.WebComponent.Safe, for: Time do
  defdelegate to_iodata(data), to: Time, as: :to_iso8601
end

defimpl Phoenix.WebComponent.Safe, for: Date do
  defdelegate to_iodata(data), to: Date, as: :to_iso8601
end

defimpl Phoenix.WebComponent.Safe, for: NaiveDateTime do
  defdelegate to_iodata(data), to: NaiveDateTime, as: :to_iso8601
end

defimpl Phoenix.WebComponent.Safe, for: DateTime do
  def to_iodata(data) do
    # Call escape in case someone can inject reserved
    # characters in the timezone or its abbreviation
    Phoenix.WebComponent.Engine.html_escape(DateTime.to_iso8601(data))
  end
end

defimpl Phoenix.WebComponent.Safe, for: List do
  ...
  def to_iodata(h) when is_integer(h) do
    raise ArgumentError,
          "lists in Phoenix.WebComponent templates only support iodata, and not chardata. Integers may only represent bytes. " <>
  ...
  def to_iodata(h) when is_binary(h) do
    Phoenix.WebComponent.Engine.html_escape(h)
  end
  ...
  def to_iodata(other) do
    raise ArgumentError,
          "lists in Phoenix.WebComponent and templates may only contain integers representing bytes, binaries or other lists, " <>
            "got invalid entry: #{inspect(other)}"
  end
end

defimpl Phoenix.WebComponent.Safe, for: Integer do
  def to_iodata(data), do: Integer.to_string(data)
end

defimpl Phoenix.WebComponent.Safe, for: Float do
  def to_iodata(data) do
    IO.iodata_to_binary(:io_lib_format.fwrite_g(data))
  end
end

defimpl Phoenix.WebComponent.Safe, for: Tuple do
